...
def get_presence_private():
	...
	for p in resp:
		if p['puuid'] == get_self_puuid():
			private = json.loads(base64.b64decode(str(p['private'])).decode("utf-8"))
			if private['isValid']:
				return private
			else:
				return { 'isValid': False, 'sessionLoopState': None, 'partyOwnerSessionLoopState': None }
...
if __name__ == '__main__':
	...
	try:
		lastGameState = None
		while True:
			presence = get_presence_private()
			if presence['isValid']:
				if ( presence['sessionLoopState'] and presence['partyOwnerSessionLoopState'] ):
					gameState = presence['partyOwnerSessionLoopState']
